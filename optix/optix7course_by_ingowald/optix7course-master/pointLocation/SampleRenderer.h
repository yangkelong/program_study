...
namespace osc {
  ...
  struct TriangleMesh {
    ...
    // 生成三角形面元集 仅供测试用
    void initMesh();
    std::vector<vec3f> vertex;  // 顶点索引坐标
    std::vector<vec3i> index;   // 三角形面元包含的三个顶点索引
    vec3f              color;   
    std::vector<vec3f> normal;   // 三角形面元的法向
    std::vector<vec2i> side_element;  // 三角形面元两侧网格单元id  -1: 表示没有网格单元
    std::vector<vec3f> query_points;  // 查询点
  };
  ...
  protected:
    ...
    std::vector<CUDABuffer> side_element_buffer;
