...
namespace osc {
  ...
  /*! add a unit cube (subject to given xfm matrix) to the current triangleMesh */
  void TriangleMesh::addUnitCube(const affine3f &xfm)
  {
    int firstVertexID = (int)vertex.size();
    vertex.push_back(xfmPoint(xfm, vec3f(0.f, 0.f, 0.f))); // 0
    vertex.push_back(xfmPoint(xfm, vec3f(1.f, 0.f, 0.f))); // 1
    vertex.push_back(xfmPoint(xfm, vec3f(0.f, 1.f, 0.f))); // 2
    vertex.push_back(xfmPoint(xfm, vec3f(1.f, 1.f, 0.f))); // 3
    vertex.push_back(xfmPoint(xfm, vec3f(0.f, 0.f, 1.f))); // 4
    vertex.push_back(xfmPoint(xfm, vec3f(1.f, 0.f, 1.f))); // 5
    vertex.push_back(xfmPoint(xfm, vec3f(0.f, 1.f, 1.f))); // 6
    vertex.push_back(xfmPoint(xfm, vec3f(1.f, 1.f, 1.f))); // 7
  ...
  OptixTraversableHandle SampleRenderer::buildAccel()
  {
    ...
    // 每套网格mesh 作为一个 buildInput
    for (int meshID = 0; meshID < meshes.size(); meshID++){
      // upload the model to the device: the builder
      TriangleMesh &model = meshes[meshID];
      vertexBuffer[meshID].alloc_and_upload(model.vertex);  // 顶点坐标数据传入设备端
      indexBuffer[meshID].alloc_and_upload(model.index);  // 面元的顶点索引数据传入设备端
