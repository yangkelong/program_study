...
namespace osc {
  ...
  void TriangleMesh::initMesh(){
    ...
    constexpr unsigned int N{100}, M{100}, K{100};
    constexpr unsigned int index_stride_i{1}, index_stride_j{N}, index_stride_k{N*M};
    // Cube 中心为原点, （Length_I, Length_J, Length_K）为右上角顶点单元坐标
    const float Length_I{1}, Length_J{1}, Length_K{1};
    const float stride_I{Length_I/(N)}, stride_J{Length_J/(N)}, stride_K{Length_K/(N)};
    int point_id = 0;
    for(unsigned int k=0; k<=K; ++k){
      for(unsigned int j=0; j<=M; ++j){
        for(unsigned int i=0; i<=N; ++i){
          // vertex(i,j,k) 
          vertex.push_back(vec3f(stride_I*i, stride_J*j, stride_K*k));
