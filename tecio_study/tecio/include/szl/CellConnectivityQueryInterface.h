...
public:
     ...
     */
    typedef uint8_t CornerIndex_t;

    /**
     * Type for indexing the faces of a cell.
     */
    typedef uint8_t FaceIndex_t;
    ...
    virtual void checkout(
        ...
        tecplot::RawArray<tecplot::ItemAddress::SubzoneAddress> const& cellSubzones) = 0;
     ...
     *     longer needed.
     */
    virtual void checkin(tecplot::RawArray<tecplot::ItemAddress::SubzoneAddress> const& cellSubzones) = 0;
     ...
     * @throws tecplot::InterruptedException if the process was interrupted
     */
    virtual void copyToUniform(struct NodeMap_s* targetNM) = 0;

    /**
     * Translates the Sizzle address to a uniform offset.
     * @param itemAddress
     *     Sizzle address needing translating.
     * @return
     *     Uniform offset translation of the Sizzle address.
     * @pre itemAddress is a Sizzle address
     * @post result is a uniform offset
     */
    virtual tecplot::ItemAddress::UniformOffset_t toUniformOffset(tecplot::ItemAddress itemAddress) = 0;

    /**
     * Translates the uniform offset to a Sizzle address.
     * @note
     ...
     * @param uniformOffset
     *     Uniform offset needing translating to its Sizzle address.
     * @return
     *     Sizzle address translation of the uniform offset.
     * @post result is a Sizzle address
     */
    virtual tecplot::ItemAddress toSZLAddr(tecplot::ItemAddress::UniformOffset_t uniformOffset) = 0;
    ...
    virtual void referencedNodeSubzones(
        tecplot::ItemAddress::SubzoneAddress                     cellSubzone,
        tecplot::RawArray<tecplot::ItemAddress::SubzoneAddress>& nodeSubzones) = 0;

    /**
     * Returns the number of cell subzones comprising the zone.
     */
    virtual tecplot::ItemAddress::SubzoneIndex_t numSubzones() = 0;
     ...
     * and the size()-1.
     */
    virtual tecplot::ItemAddress::ItemOffset_t size(tecplot::ItemAddress::SubzoneAddress cellSubzone) = 0;
     ...
     */
    virtual size_t nodeCount(tecplot::ItemAddress cellAddress) = 0;
     ...
     *     Number of nodes for each cell in the cell's subzone.
     */
    virtual size_t faceCount(tecplot::ItemAddress cellAddress) = 0;

    /**
     * Returns the node addresses for the cell using direct access to the connectivity maintained by
     * the addressed cell subzone. The node addresses can then be used for direct access to the node
     * values.
     * @param cellAddress
     *     Index/offset address within the cell subzone or a uniform address for which the node
     *     address is desired.
     * @param nodeAddresses
     *     Array into which the resulting cell's node addresses are placed. The addressing of the
     *     nodes returned is the same as that of the cellAddress.
     * @pre nodeAddresses.empty()
     * @throws std::bad_alloc if sufficient memory is not available
     */
    virtual void nodeAddresses(
        tecplot::ItemAddress                     cellAddress,
        tecplot::RawArray<tecplot::ItemAddress>& nodeAddresses) = 0;

    /**
     * Returns the node address for the cell corner using direct access to the connectivity
     * maintained by the addressed cell subzone. The node address can then be used for direct access
     * to the node value.
     * @param cellAddress
     *     Index/offset address within the cell subzone or a uniform address for which the node
     *     address is desired.
     * @param cellCorner
     *     Indicates which node within the cell is desired.
     * @return
     *     Node address of the specified addressed cell's corner. The addressing of the node
     *     returned is the same as that of the cellAddress.
     */
    virtual tecplot::ItemAddress nodeAddress(
        tecplot::ItemAddress cellAddress,
        CornerIndex_t        cellCorner) = 0;
     ...
     * @throws std::bad_alloc if sufficient memory is not available
     */
    virtual void updateInternalFaceNeighbors(tecplot::RawArray<tecplot::ItemAddress::SubzoneAddress> const& cellSubzones) = 0;
    ...
    virtual void faceNeighbors(
        ...
        tecplot::RawArray<tecplot::ItemAddress>& cellNeighbors,
    ...
    virtual void setFaceNeighbors(
        tecplot::ItemAddress                           cellAddress,
        FaceIndex_t                                    cellFace,
        tecplot::RawArray<tecplot::ItemAddress> const& cellNeighbors,
